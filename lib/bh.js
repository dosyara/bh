...
function BH() {
    ...
    this.utils = {
        ...
        bind: function(bind, force) {
            if (bind !== undefined) {
                this.ctx.bind = this.ctx.bind === undefined || force ? bind : this.ctx.bind;
                return this;
            } else {
                return this.ctx.bind;
            }
        }
    };
}
...
BH.prototype = {
    ...
    buildMatcher: function() {
        ...
        function applyBinders(ctx, json) {
            var controlList = { checkbox: 1, input: 1, select: 1 },
                bindsParams = [];

            function callBind(type, bind) {
                if (typeof _binds[type] === 'undefined') {
                    throw new Error('unknown bind type: ' + type);
                }

                return _binds[type](ctx, json, bind);
            }

            if (!Array.isArray(json.bind)) {
                json.bind = [json.bind];
            }

            json.bind.forEach(function(bind) {
                if (bind.model || bind.modelData) {
                    callBind('model', bind);
                } else {
                    if (typeof bind.type === 'undefined' && controlList[json.block]) {
                        bind.type = json.block;
                    }

                    bindsParams.push(bind);

                    callBind(bind.type, bind);
                }
            }, this);

            if (bindsParams.length) {
                ctx.mix({
                    block: ctx.tParam('gluedBlock'),
                    elem: 'model-field',
                    js: bindsParams
                });
            }
        }
        res.push(applyBinders.toString());

        /* binders start */
        res.push('if (json.bind) {');
        res.push('    applyBinders(ctx, json);');
        res.push('}');
        /* binders end */
    ...
    processBemJson: function(bemJson, blockName, ignoreContent) {
        ...
        var compiledMatcher = (this._fastMatcher || (this._fastMatcher = Function('ms', '_binds', this.buildMatcher())(this._matchers, this._binds)));
    ...
    declBind: function(type, binder) {
        this._binds[type] = binder;
    }
};
